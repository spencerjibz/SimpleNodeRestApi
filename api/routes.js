  ...
  jwt.verify(req.token, config.TOKEN_SEC, (err, authdata) => {
    if (err) {
      ...
      User.getUserByUsername(authdata.user.username, (err, user) => {
        err ? res.sendStatus(404) : delete user._doc.password;
  ...
  if (test) {
    ...
    User.createUser(newUser, function (err, user) {
      if (err) {
        res.json({
          code: 404,
          message: `failed to create new user because the user already exists`,
        });
      } else {
...
router.post("/deleteone", (req, res) => {
  ...
  User.deleteUser(req.body.email, (err, isdone) => {
    ...
    if (isdone) {
      ...
      res.json({
        code: 404,
        message: " Failed to delete account, username is incorrect",
      });
    }
  });
});
  ...
  jwt.verify(req.params.token, config.TOKEN_SEC, (err, data) => {
    if (err) {
      ...
      User.changedinfo(data.user.username, password, (err, isdone) => {
        if (err) {
          res.json({
            message: "password enter the right password",
          });
        } else {
          res.json({
